...
/** A card with a price tag above it, the way a shop screenshot has one. */
function shopCard(text: string | null, scale = 2) {
  ...
  if (text) drawPriceTag(screen, box.x0 + 40, box.y0 - 40, scale, text, digits);
  return { screen, box };
}
...
describe('readPrice', () => {
  ...
  it('refuses a tag it cannot read rather than guessing a number', () => {
    const { screen, box } = shopCard('$7');
    // Scribble over the digit, leaving the tag and its dollar sign in place.
    for (let y = box.y0 - 40; y < box.y0 - 10; y++) {
      for (let x = box.x0 + 60; x < box.x0 + 80; x++) {
